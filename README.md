...
## **INTRODUCTION**
The challenge centered on analyzing a dataset from Olist, a popular e-commerce company in Brazil. This dataset included important details about different aspects of Olist's operations, like product listings, sellers, and customer interactions. My goal was to explore the dataset and find valuable insights based on specific questions. The challenge organizers carefully curated and prepared the dataset to help participants, including me, extract meaningful information and gain insights related to Olist's business.
...
.With the messy table on the left and the cleaned table on the right, the cleaning process included several steps to make sure the data was correct and consistent. The messy table showed inconsistencies, missing values, duplicates, and other problems that could affect the analysis. However, after using thorough data cleaning methods, the cleaned table now offers a more organized and trustworthy dataset. The cleaning process involved filling in or removing missing values, standardizing formats, correcting inconsistencies, removing duplicates, and fixing any other data quality problems. As a result, the cleaned table serves as a strong base for conducting meaningful analysis and gaining valuable insights from the dataset.
